/**
 * $Id: editor_plugin_src.js 144 2006-11-14 21:42:06Z spocke $
 *
 * @author Moxiecode
...
var TinyMCE_ContextMenuPlugin = {
	...
	initInstance : function(inst) {
		...
		if (tinyMCE.isMSIE || tinyMCE.isOpera || tinyMCE.isMSIE7)
			return;
	...
	_showContextMenu : function(e, inst) {
		...
		window.setTimeout(function () {
			...
			if (sel || (elm ? (elm.nodeName == 'A' && tinyMCE.getAttrib(elm, 'name') == '') || (elm.nodeName == 'IMG') : false)) {
				...
				var linkItem = 'mceLink';
				if(inst.hasPlugin("advlink")){
					linkItem = 'mceAdvLink';
				}else if(inst.hasPlugin("advlink_se")){
					linkItem = 'mceAdvLinkSE';	
				}
				contextMenu.addItem(tinyMCE.baseURL + "/themes/advanced/images/link.gif", "$lang_link_desc", linkItem);
			...
			if (elm) {
				switch (elm.nodeName) {
					case "IMG":
						...
						if ((tinyMCE.hasPlugin('mediamanager') || tinyMCE.hasPlugin('mediamanager_se')) && /mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(tinyMCE.getAttrib(elm, 'class'))){
							contextMenu.addItem(tinyMCE.baseURL + "/plugins/mediamanager/images/mediamanager.gif", "$lang_mediamanager_title", "mceMedia");
						}else{
							var imgItem = 'mceImage';
							if(inst.hasPlugin("imgmanager")){
								imgItem = 'mceImageManager';
							}else if(inst.hasPlugin("imgmanager_se")){
								imgItem = 'mceImageManagerSE';	
							}
							contextMenu.addItem(tinyMCE.baseURL + "/themes/" + theme + "/images/image.gif", "$lang_image_props_desc", imgItem);
						}
						break;

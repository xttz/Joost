...
function generateTemplateColors() {
	...
	var template_colors = tinyMCE.getParam('template_colors');
	...
	h += '<table border="0" cellspacing="1" cellpadding="0">'
		+ '<tr>';
	if(template_colors != ''){
		template_colors = template_colors.split(',');	
		for (i=0; i<template_colors.length; i++) {
			h += '<td bgcolor="' + template_colors[i] + '">'
				+ '<a href="javascript:selectColor();" onfocus="showColor(\'' + template_colors[i] +  '\');" onmouseover="showColor(\'' + template_colors[i] +  '\');">'
				+ '<img border="0" src="images/spacer.gif" width="10" height="10" title="' + template_colors[i] +  '" alt="' + template_colors[i] +  '" /></a></td>';
			if ((i+1) % 18 == 0)
				h += '</tr><tr>';
		}
	}else{
		h += '<td>' + tinyMCE.getLang('lang_no_template_colors', '', false); + '</td></tr>'	
	}
	h += '</table>';
